...
export function ArcadeApp() {
    ...
    return (
      <div className="flex h-full flex-col bg-bg">
        <div className="flex items-center gap-3 border-b-2 border-line bg-surface-2 px-3 py-2">
          ...
          <span className="font-display text-[10px] text-ink">{playing.file}</span>
          <span className="font-mono text-[11px] text-faint max-sm:hidden">
            {playing.controls}
          </span>
          <button
            onClick={() => setPlaying(null)}
            className="ml-auto border-2 border-line px-2.5 py-1 font-display text-[9px] text-dim transition-colors hover:border-accent hover:text-accent"
          >
            eject
          </button>
        </div>
  ...
  return (
    <div className="os-scroll h-full overflow-y-auto bg-surface p-6">
      <h2 className="font-display text-base text-ink">Arcade</h2>
        ...
        {GAMES.map((g) => (
          ...
          >
            <div className="min-w-0">
              <div className="font-display text-[11px] text-ink">{g.file}</div>
              <div className="mt-1.5 font-mono text-[11px] text-faint">{g.meta}</div>
            </div>
            <span className="ml-auto shrink-0 border-2 border-accent/60 bg-accent-soft px-3.5 py-1.5 font-display text-[9px] text-accent transition-colors group-hover:bg-accent group-hover:text-bg">
              ▶ PLAY
            </span>
          </button>
        ))}
